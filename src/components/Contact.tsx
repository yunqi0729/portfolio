                ...
                <div className={`rounded-2xl px-4 py-6 sm:px-5 sm:py-7 border-2 flex flex-col gap-4 border-black ${isDarkMode ? 'bg-[#EDC89B]' : 'bg-white'}`}>
                    <div className="relative">
                        <button className="w-full flex items-center gap-3 rounded-xl cursor-pointer group text-left" onClick={handleCopyEmail}>
                            <span className="text-base sm:text-md md:text-lg font-semibold flex-shrink-0">📮</span>
                            <div className="flex-1 min-w-0">
                                <span className={`text-base sm:text-md md:text-lg font-semibold break-all inline-block transition-transform duration-200 hover:scale-105 ${isDarkMode ? 'text-[#C37448] hover:text-[#9C562F]' : 'text-[#3F4E79] hover:text-[#122448]'}`}>{email}</span>
                            </div>
                        </button>
                        {copied && (
                            <div className={`absolute -top-8 left-1/2 -translate-x-1/2 px-4 py-1.5 rounded-lg text-sm font-medium shadow-lg whitespace-nowrap z-10 ${isDarkMode ? 'bg-[#9C562F] text-white' : 'bg-[#122448] text-white'}`}>✓ Email copied to clipboard!</div>
                        )}
                    </div>
                    ...
                    <a className="flex items-center gap-3 rounded-xl group" href={linkedin} target="_blank" rel="noopener noreferrer">
                        <span className={`text-base sm:text-md md:text-lg font-semibold flex-shrink-0 ${isDarkMode ? 'text-[#9C562F]' : 'text-[#122448]'}`}>🔗</span>
                        <div className="flex-1 min-w-0">
                            <span className={`text-base sm:text-md md:text-lg font-semibold underline underline-offset-2 break-all inline-block transition-transform duration-200 hover:scale-105 ${isDarkMode ? 'text-[#C37448] hover:text-[#9C562F]' : 'text-[#3F4E79] hover:text-[#122448]'}`}>{linkedin}</span>
                        </div>
                    </a>
                    <div className={`border-t-2 border-dotted mx-2 ${isDarkMode ? 'border-[#9C562F]' : 'border-[#122448]'} opacity-30`} />
                    <a className="flex items-center gap-3 rounded-xl group" href={portfolio} target="_blank" rel="noopener noreferrer">
                        <span className={`text-base sm:text-md md:text-lg font-semibold flex-shrink-0 ${isDarkMode ? 'text-[#9C562F]' : 'text-[#122448]'}`}>💻</span>
                        <div className="flex-1 min-w-0">
                            <span className={`text-base sm:text-md md:text-lg font-semibold underline underline-offset-2 break-all inline-block transition-transform duration-200 hover:scale-105 ${isDarkMode ? 'text-[#C37448] hover:text-[#9C562F]' : 'text-[#3F4E79] hover:text-[#122448]'}`}>{portfolio}</span>
                        </div>
                    </a>
                </div>
            </div>
